...
use bmw_routing_engine::{graph::*, io::*, rank_select_map::*};
...
fn main() {
    ...
    new_first_out
        .write_to(out_path.join("first_out").to_str().unwrap())
        .expect("could not write first_out");
    new_head.write_to(out_path.join("head").to_str().unwrap()).expect("could not write head");
    new_travel_time
        .write_to(out_path.join("travel_time").to_str().unwrap())
        .expect("could not write travel_time");
    new_lat.write_to(out_path.join("latitude").to_str().unwrap()).expect("could not write latitude");
    new_lng
        .write_to(out_path.join("longitude").to_str().unwrap())
        .expect("could not write longitude");
}
