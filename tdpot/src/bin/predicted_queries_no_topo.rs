...
fn main() -> Result<(), Box<dyn Error>> {
    let _reporter = enable_reporting("predicted_queries_no_topo");
    ...
    match pot.as_deref() {
        Some("interval_min_pot") => {
            ...
            let mut server = without_reporting(|| Server::new_no_topo(&graph, interval_min_pot, TDDijkstraOps::default()));
        ...
        Some("multi_metric_pot") => {
            ...
            let mut server = without_reporting(|| Server::new_no_topo(&graph, mm_pot, TDDijkstraOps::default()));
        ...
        Some("zero") => {
            ...
            let mut server = without_reporting(|| Server::new_no_topo(&graph, rust_road_router::algo::a_star::ZeroPotential(), TDDijkstraOps::default()));
        ...
        _ => {
            ...
            let mut server = without_reporting(|| Server::new_no_topo(&graph, cch_pot_data.forward_potential(), TDDijkstraOps::default()));
