...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    match variant.as_deref() {
        ...
        Some("iterative_detour_blocking") => {
            let mut server = HeuristicTDTrafficAwareServer::new(smooth_graph.borrowed(), &graph, &smooth_cch_pot, imp);
        ...
        Some("iterative_path_fixing") => {
            let mut server = IterativePathFixing::new(smooth_graph.borrowed(), &graph, &smooth_cch_pot, imp);
        ...
        Some("smooth_path_baseline") => {
            let mut server = SmoothPathBaseline::new(smooth_graph.borrowed(), &graph, &smooth_cch_pot, imp);
