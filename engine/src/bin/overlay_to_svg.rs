...
fn main() {
    ...
    let contraction_count = args.next().map(|s| s.parse::<usize>().unwrap_or_else(|_| panic!("could not parse {} as number of contracted nodes", s))).unwrap();
        ...
        if in_bounding_box(node) {
            ...
            for link in up.neighbor_iter(node_order.rank(node as NodeId)) {
                ...
                if in_bounding_box(link_node) {
                    println!("<line x1=\"{}\" y1=\"{}\" x2=\"{}\" y2=\"{}\" class=\"arc\" />", lng[node], lat[node], lng[link_node], lat[link_node]);
                }
            }
            ...
            for link in down.neighbor_iter(node_order.rank(node as NodeId)) {
                let link_node = node_order.node(link.node) as usize;
                if in_bounding_box(link_node) {
                    println!("<line x1=\"{}\" y1=\"{}\" x2=\"{}\" y2=\"{}\" class=\"arc\" />", lng[link_node], lat[link_node], lng[node], lat[node]);
                }
            }
        }
    }
