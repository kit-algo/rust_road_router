...
fn main() {
    ...
    data.functional_road_classes.write_to(Path::new(out_dir).join("functional_road_classes").to_str().unwrap()).expect("writing lat failed");
    data.lat.write_to(Path::new(out_dir).join("latitude").to_str().unwrap()).expect("writing lat failed");
    data.lng.write_to(Path::new(out_dir).join("longitude").to_str().unwrap()).expect("writing lng failed");
    data.link_id_mapping.write_to(Path::new(out_dir).join("link_id_mapping").to_str().unwrap()).expect("writing link_id_mapping failed");
    data.here_rank_to_link_id.write_to(Path::new(out_dir).join("here_rank_to_link_id").to_str().unwrap()).expect("writing here_rank_to_link_id failed");
}
