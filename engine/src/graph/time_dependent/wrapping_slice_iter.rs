...
impl<'a> WrappingSliceIter<'a> {
    pub fn new(slice: &'a [TTIpp], range: WrappingRange) -> WrappingSliceIter<'a> {
        if slice.is_empty() {
            return WrappingSliceIter { range, slice, next_index: None, initial_index: 0 }
        }

        if let Some(last) = slice.last() {
            debug_assert!(last.at < period());
        }
...
mod tests {
    ...
    fn test_full_range_starting_in_middle() {
        run_test_with_periodicity(10, || {
            let range = WrappingRange::new(Range { start: 5, end: 5 });
            let vec: Vec<_> = vec![(0,0), (2,5), (8,7), (9,4)].into_iter().map(|(at, val)| TTIpp::new(at, val)).collect();
    ...
    fn test_full_range_starting_on_elem() {
        run_test_with_periodicity(10, || {
            let range = WrappingRange::new(Range { start: 8, end: 8 });
            let vec: Vec<_> = vec![(0,0), (2,5), (8,7), (9,4)].into_iter().map(|(at, val)| TTIpp::new(at, val)).collect();
    ...
    fn test_partial_range() {
        run_test_with_periodicity(10, || {
            let range = WrappingRange::new(Range { start: 5, end: 1 });
            let vec: Vec<_> = vec![(0,0), (2,5), (8,7), (9,4)].into_iter().map(|(at, val)| TTIpp::new(at, val)).collect();
