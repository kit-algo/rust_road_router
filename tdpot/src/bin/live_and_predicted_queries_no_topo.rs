...
fn main() -> Result<(), Box<dyn Error>> {
    let _reporter = enable_reporting("live_and_predicted_queries_no_topo");
    ...
    match pot.as_deref() {
        Some("interval_min_pot") => {
            ...
            let mut server = without_reporting(|| Server::new_no_topo(&live_graph, interval_min_pot, PessimisticLiveTDDijkstraOps::default()));
        ...
        Some("multi_metric_pot") => {
            ...
            let mut server = without_reporting(|| Server::new_no_topo(&live_graph, mm_pot, PessimisticLiveTDDijkstraOps::default()));
        ...
        _ => {
            ...
            let mut server = without_reporting(|| Server::new_no_topo(&live_graph, cch_pot_data.forward_potential(), PessimisticLiveTDDijkstraOps::default()));
