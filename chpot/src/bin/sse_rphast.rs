    ...
    for target_set_size_exp in [14] {
        ...
        for ball_size_exp in 21..=21 {
            ...
            for (sources, targets) in &queries {
                ...
                {
                    let _alg_ctx = algos_ctxt.push_collection_item();
                    report!("algo", "sse_rphast_query");
                    let (_rphast_result, time) = measure(|| rphast_query.query(targets, &rphast));
                    // let (_rphast_result, time) = measure(|| {
                    //     for chunk in targets.array_chunks::<16>() {
                    //         rphast_query.query(chunk, &rphast);
                    //     }
                    // });
                    report!("running_time_ms", time.to_std().unwrap().as_nanos() as f64 / 1_000_000.0);
