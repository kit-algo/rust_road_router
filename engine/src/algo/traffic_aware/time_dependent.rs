...
pub struct TDTrafficAwareServer<'a, P> {
    ...
    live_pot: P,
    live_graph: &'a time_dependent::TDGraph,
                ...
                label_idx = self.dijkstra_data.distances[*parent as usize]
                    ...
                    .position(|NodeQueueLabelOrder(l)| {
                        let min_weight_edge = self
                            ...
                            .min_by_key(|&EdgeIdT(edge_id)| self.live_graph.travel_time_function(edge_id).eval(l.0))
                            .unwrap();

                        l.0 == dist - self.live_graph.travel_time_function(min_weight_edge.0).eval(l.0) && l.1.is_subset(parent_active_forb_paths)
                    })
                    .unwrap();
...
pub struct HeuristicTDTrafficAwareServer<'a, P> {
    ubs_checker: MinimalNonShortestSubPaths<'a>,
    shortest_path: TDTopoDijkServer<TDGraph, BlockedDetoursDijkstra<TDDijkstraOps>, td_astar::TDRecyclingPotential<P>, true, true, true>,
}

impl<'a> HeuristicTDTrafficAwareServer<'a, BorrowedCCHPot<'a>> {}

impl<'a, P: td_astar::TDPotential> HeuristicTDTrafficAwareServer<'a, P> {
    pub fn new(smooth_graph: BorrowedGraph<'a>, live_graph: &'a time_dependent::TDGraph, smooth_cch_pot: &'a CCHPotData, live_cch_pot: P) -> Self {
        let n = live_graph.num_nodes();
        ...
        Self {
            ...
            shortest_path: TDTopoDijkServer::new(
                ...
                BlockedDetoursDijkstra::new(n, TDDijkstraOps()),
            ),
        }
    }

    pub fn query(&mut self, query: TDQuery<time_dependent::Timestamp>, epsilon: f64, mut path_cb: impl FnMut(&[NodeId])) -> Option<()> {
        let timer = Timer::new();
...
pub struct IterativePathFixing<'a, P> {
    ubs_checker: MinimalNonShortestSubPaths<'a>,
    shortest_path: TDTopoDijkServer<TDGraph, TDDijkstraOps, P, true, true, true>,
    live_graph: &'a TDGraph,
}

impl<'a, P: TDPotential> IterativePathFixing<'a, P> {
    pub fn new(smooth_graph: BorrowedGraph<'a>, live_graph: &'a TDGraph, smooth_cch_pot: &'a CCHPotData, live_cch_pot: P) -> Self {
        let _blocked = block_reporting();
        Self {
            ubs_checker: MinimalNonShortestSubPaths::new(smooth_cch_pot, smooth_graph),
            shortest_path: TDTopoDijkServer::new(live_graph, live_cch_pot, TDDijkstraOps()),
            live_graph,
        }
    }
...
pub struct SmoothPathBaseline<'a, P> {
    shortest_live_path: TDTopoDijkServer<TDGraph, TDDijkstraOps, P, true, true, true>,
...
impl<'a, P: TDPotential> SmoothPathBaseline<'a, P> {
    pub fn new(smooth_graph: BorrowedGraph<'a>, live_graph: &'a TDGraph, smooth_cch_pot: &'a CCHPotData, live_cch_pot: P) -> Self {
        let _blocked = block_reporting();
        Self {
            shortest_live_path: TDTopoDijkServer::new(live_graph, live_cch_pot, TDDijkstraOps()),
...
pub fn path_dist_iter<'a>(path: &'a [NodeId], departure: Timestamp, graph: &'a TDGraph) -> impl Iterator<Item = Weight> + 'a {
    std::iter::once(0).chain(path.windows(2).scan(0, move |state, node_pair| {
        ...
        for EdgeIdT(edge_id) in graph.edge_indices(node_pair[0], node_pair[1]) {
            min_edge_weight = std::cmp::min(min_edge_weight, graph.travel_time_function(edge_id).eval(*state + departure));
        }
