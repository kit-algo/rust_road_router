...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    report!("program", "chpot_turns_td_live");
    ...
    let graph = TDGraph::new(first_out, head.clone(), first_ipp_of_arc, ipp_departure_time, ipp_travel_time);
    ...
    for _i in 0..num_queries {
        ...
        report!("to", to);
        report!("at", now);
        let (mut res, time) = measure(|| TDQueryServer::query(&mut server, TDQuery { from, to, departure: now }));
