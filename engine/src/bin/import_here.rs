...
fn main() {
    ...
    let (graph, lat, lng, link_indices) = here::read_graph(&source);
    ...
    link_indices.write_to(Path::new(out_dir).join("link_indices").to_str().unwrap()).expect("writing link_indices failed");
}
