...
fn main() {
    ...
    let min_lat = (args.next().map(|s| s.parse::<f64>().expect(&format!("could not parse {} as lat coord", s))).unwrap_or(-360.0) * 100_000.) as i64;
    let min_lon = (args.next().map(|s| s.parse::<f64>().expect(&format!("could not parse {} as lon coord", s))).unwrap_or(-360.0) * 100_000.) as i64;
    let max_lat = (args.next().map(|s| s.parse::<f64>().expect(&format!("could not parse {} as lat coord", s))).unwrap_or(360.0) * 100_000.) as i64;
    let max_lon = (args.next().map(|s| s.parse::<f64>().expect(&format!("could not parse {} as lon coord", s))).unwrap_or(360.0) * 100_000.) as i64;
